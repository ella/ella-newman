...
from ella_newman.changelist import NewmanChangeList, FilterChangeList
from ella_newman import fields, widgets, utils
from ella_newman.models import DenormalizedCategoryUserRole, AdminUserDraft, AdminHelpItem
from ella_newman.decorators import require_AJAX
from ella_newman.permission import is_category_model, model_category_fk, model_category_fk_value, applicable_categories
from ella_newman.permission import has_category_permission, get_permission, permission_filtered_model_qs, is_category_fk
from ella_newman.forms import DraftForm
from ella_newman.xoptions import XModelAdmin
...
def thumb_html(photo):
    if not photo:
        return '<span class="red">(None)</span>'
    else:
        return '<span class="widget-thumb"><img src="%s" alt="Thumbnail %s" /></span>' % (photo.get_formated_photo(get_thumb_format())['url'], photo.title)
